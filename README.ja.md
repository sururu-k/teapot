...
provider → APIキー → モデル → 最初のエージェントの順に入力するだけ。
設定ファイルは自動で書かれる（手編集不要）。

<details>
<summary>自分で設定を書きたい場合</summary>
...
設定ファイルが存在するとウィザードはスキップされ、エージェントが即座に始動します。
...
- 入力欄に指示を入力（例：「このバグを直してテストも通了せ」）
- エージェントがファイル読み書き・bash実行・テストを自走します

便利キー: `t` ターミナル · `d` 右パネル · `/` コマンド · `Esc` 中断
...
各エージェントは独立したワークスペースを持ち、Discord風UIのチャンネルとして動きます。
Claude CodeやCodex CLIのラッパーではなく、**teapot自身がエージェントルーループ
(read_file / edit_file / apply_patch / bash / read_url / memory / skills) を実装**しています。
...
- 定期タスク（例：「毎30分 git status とテストを確認して報告」）
...
- 既定ではlocalhost限定listen。LAN公開する場合は **必ず** `--host 0.0.0.0` と同時に
  `TEAPOT_API_TOKEN`（または設定の `password`）を設定:
...
| `agents[].provider` / `model` | 既定値 | エージェント事の上書き |
...
| `contextTokenBudget` | 派生 | compact開始闘値（トークン数）。null/未設定=75%派生を推奨 |
...
モデルのコンテキスト窓は起動時に `/v1/models` から自動推定され、
runtimeパネルの使用率ゲージとcompact派生に反映されます。
...
トークン予算（既定: ウィンドウの75%、不明時96k）を超えると、
古いターンをLLM要約で圧縮する。Goal/Memory/Todoは別ファイル保持なので:
...
各イベント敢 `parent` で連結、フォークは分岐点を共有。
`goal.md` `memory.md` `todo.md` も同ディレクトリに保存される。
